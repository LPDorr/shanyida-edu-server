...
// 验证用户名是否存
router.post('/valid_name/:user_name', Token.verify, function(req, res, next) {
...
// 新增用户
router.post('/add/:user_name', Token.verify, function(req, res, next) {
	var user_name = req.params.user_name;
	var sql = "insert `dt_user`(`user_name`,`user_pwd`) values(?,'123');";
	res.flag = query(sql, [ user_name ])
		.then(results => httpResult.success());
	next();
});
// 管理员重置用户密码
router.post('/resetpwd/:user_name', Token.verify, function(req, res, next) {
	var user_name = req.params.user_name;
	var sql = "update `dt_user` set `user_pwd` = '123' where `user_name` = ?;";
	res.flag = query(sql, [  user_name ])
		.then(results => httpResult.success());
	next();
});
